...
ListObj *newList(void) {
    ListObj *new_list = malloc(sizeof(ListObj));
    if (new_list) {
        new_list->head = NULL;
        new_list->length = 0;
    }
...
int delElement(List L, int i) {
    ...
    if (i == 0) {
        ...
        L->head = next_node; // set the head of the linked list to the head->next of the linked list
    ...
    // if node is in the middle
        ...
        for (int node = 0; node < L->length; node++) {
            if (node == i - 1) { // because we want to delete the node when next_node is at index i not when curr_node is at index i
            curr_node->next = next_node->next; // set the curr_node's next to next_node's next
            int data = next_node->data; // save the data of the node we are going to delete
